 ...
 * Evolution: loser of interaction's losing strategy parameter falls subject to mutation (i.e., noise wrt epsilon)
    ...
    public static void main(String[] args) throws IOException {
        ...
        while(round != max_num_rounds){
            ...
            while(players_this_round.size() != 0){  // while there are players left unassigned
                ...
                proposer.playUG(responder);  // playUG UG

                // start evolution
                if(proposer.getScore()>responder.getScore()){
//                    responder.setStrategy(proposer.getP(),proposer.getQ());
                    responder.setQ(ThreadLocalRandom.current().nextDouble(responder.getQ()-epsilon, responder.getQ()+epsilon));
                } else if(responder.getScore()>proposer.getScore()){
//                    proposer.setStrategy(responder.getP(),responder.getQ());
                    proposer.setP(ThreadLocalRandom.current().nextDouble(proposer.getP()-epsilon, proposer.getP()+epsilon));
                }
                // end evolution
